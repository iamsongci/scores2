					...
					<c:forEach items="${notifyList}" var="notify">
						<tbody>
							<tr>
								...
								<td><small> <c:if
											test="${notify.toStudent eq false}">否</c:if> <c:if
											test="${notify.toStudent eq true}">是</c:if>
								</small></td>
								<%-- <td>
									<div class="visible-md visible-lg hidden-sm hidden-xs">
										<a data-target="#more" data-toggle="modal"
											onclick="init('${notify.title}', '${notify.content}')"> <i
											class="fa fa-info-circle"></i> <small>${notify.title}</small>
										</a>
									</div>
								</td> --%>
								<td>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div style="padding: 10px 50px 10px 30px">
						...
						<!-- <input type="text" id="newtime" name="newtime" style="float: right;"
							placeholder="时间。例:2016-1-12"> -->
					</div>
