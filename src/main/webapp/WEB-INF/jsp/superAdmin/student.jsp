...
<script type="text/javascript">
	...
	function del(stuID) {
		if (confirm('确认删除?')) {
			$.ajax({
				type : "post",
				url : "./${sessionScope.pathCode}/delStu.do",
				data : "stuID=" + stuID,
				dataType : 'html',
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div style="padding: 10px 30px 10px 30px">
						...
						<img src="./img/type-stu.png" class="img-responsive"
			...
			<div class="panel-body">
				<table class="table  table-hover">
					<thead>
						<tr>
							...
							<button data-target="#upload" data-toggle="modal" type="button"
								class="btn btn-info"
								style="float: right; margin: 5px 10% 5px 5px;">导入学生基本信息</button>
						</tr>
						<tr>
							<th><small>学号</small></th>
							<th><small>姓名</small></th>
							<th><small>性别</small></th>
							<th><small>班级ID</small></th>
					...
					<c:forEach items="${studentList}" var="student">
						<tbody>
							<tr>
								<td><small>${student.studentID}</small></td>
								<td><small>${student.studentName}</small></td>
								<td><small>
									<c:if test="${student.studentSex eq true}">
										女
									</c:if>
									<c:if test="${student.studentSex eq false}">
										男
									</c:if>
								</small></td>
								<td><small>${student.studentClassID }</small></td>
								<td><small>
									<c:if test="${student.hasPro eq null }">
										<button type="button" class="btn btn-danger" onclick="del('${student.studentID}')">删除</button>
									</c:if>
								</small></td>
