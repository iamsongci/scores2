	...
	<div class="row">
		<div class="col-sm-12">
			...
			<ol class="breadcrumb">
				...
				<li class="active">导师分配权限</li>
			</ol>
			<div class="page-header">
				<h3>
					导师分配权限管理/<a href="./${sessionScope.pathCode}/selectTutorPower2.do">成绩汇总权限</a>
				</h3>
			</div>
					...
					<c:forEach items="${tutorInfoList}" var="tutor">
						<tbody>
							<tr>
								...
								<td><small>${tutor.tutorPhone }</small></td>
								<td><c:if test="${tutor.projectName eq null }">
										<small>无</small>
									</c:if> <c:if test="${tutor.projectName ne null }">
										<small>${tutor.projectName}</small>
									</c:if></td>
								<td>
									<div class="visible-md visible-lg hidden-sm hidden-xs">
										<a
											href="./${sessionScope.pathCode}/updateTutorInfoA.do?tutorID=${tutor.tutorID}"
											data-toggle="modal" data-target="#pwdM"> <i
											class="fa fa-info-circle"></i> <small>修改权限</small>
										</a> <a href="javaScript:void(0);" data-toggle="modal"
											data-target="#pwdM"> <i class="fa fa-info-circle"></i> <small
											onclick="tutorId(${tutor.tutorID})">重置权限</small>
										</a>
									</div>
								</td>
							</tr>
